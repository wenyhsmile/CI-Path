...
from utils import hetero_src_tgt_khop_in_subgraph, get_neg_path_score_func, k_shortest_paths_with_max_length,get_community,get_community_multi
...
def remove_edges_except_k_core_graph2(ghomo, k, always_preserve=[]):
    '''
...
class PaGELink(nn.Module):
    ...
    def path_loss(self, src_nid, tgt_nid, g, eweights, num_paths=5):
        ...
        loss_differ = max(0,loss_off_path+0.2+loss_on_path)
    ...
    def get_edge_mask(self, 
        ...
        #communitiesnode = get_community(ml_ghomo)
        #print(111,communitiesnode)
        #print(222,ml_ghomo)
        
        #print(00,ml_edge_mask_dict)
        ...
        for e in range(self.num_epochs): 
            ...
            ml_eweight_dict_ori = {etype: ml_edge_mask_dict[etype].sigmoid() for etype in ml_edge_mask_dict}
            #print(111,ml_eweight_dict_ori)
            
            #import pdb ;pdb.set_trace()
            #print(100,ml_eweight_dict_ori )
            ml_ghetero.edata['eweight'] = ml_eweight_dict_ori
            ml_ghomo = dgl.to_homogeneous(ml_ghetero, edata=['eweight'])
            #ml_ghomo_eweights = ml_ghomo.edata['eweight']
            community = get_community(ml_ghomo)
            ntype_hetero_nids_to_homo_nodeids = get_ntype_hetero_nids_to_homo_nids(ml_ghetero)       
            ...
            for etype in ml_edge_mask_dict:
              #ml_eweight_dict[etype]= ml_edge_mask_dict[etype].sigmoid()
              #ml_eweight_dict[etype]= ml_eweight_dict_ori[etype].detach()
              
              ml_eweight_dict[etype] = ml_eweight_dict_ori[etype]
              #print(111,ml_eweight_dict[etype])
              
              edges_score[etype] = torch.Tensor(ml_eweight_dict[etype].size()[0]).fill_(0).cuda()
              #print(111,edges_score[etype]) 
              ...
              for i, (src_nodeid, tgt_nodeid) in enumerate(zip(src_nodeids.tolist(), tgt_nodeids.tolist())):
                  homo_src_nodeid = ntype_hetero_nids_to_homo_nodeids[(src_nodetype, src_nodeid)]
                  homo_tgt_nodeid = ntype_hetero_nids_to_homo_nodeids[(tgt_nodetype, tgt_nodeid)]
                  
                  if community[homo_src_nodeid] != community[homo_tgt_nodeid]:
                      edges_score[etype][i] = 0.1  
               
              #print(222,edges_score[etype])  
                        
              #ml_eweight_dict[etype] = torch.mul(ml_eweight_dict[etype], edges_score[etype])  
              ml_eweight_dict[etype] = ml_eweight_dict[etype] - edges_score[etype]
              #print(333,etype,ml_eweight_dict[etype]) 
              
                         
            #print(333,ml_eweight_dict  )
            ...
            #print(l)
            #print(3,ml_edge_mask_dict)
            #print(4,ml_eweight_dict)
            #print(5,ml_ghomo)
            #print(6,ml_ghomo_eweights)
            #communitiesnode = get_community_multi(ml_ghomo)
            
            
            #communitiesnode = get_community(ml_ghomo)
            #print(1,communitiesnode)
    ...
    def get_paths(self,
        ...
        #print(111,communities)
        #community=get_community(ghomo)
        #print(222,community)
        #print(333,ghomo)
        ...
        community=get_community(ghomo)
        #print(1110000000000000000000000,community)
    
        if len(homo_paths) > 0:
            for homo_path in homo_paths:
            
                com_num=len(set(community[node] for node in homo_path)) 
                com=[community[node] for node in homo_path]
