...
def get_community_multi(g):
    g_nx = dgl.to_networkx(g.cpu(), node_attrs=[dgl.NTYPE]).to_undirected()
   
    adj_matrix = nx.to_numpy_array(g_nx)
   
    g_ig = ig.Graph.Adjacency(adj_matrix.tolist(), mode="undirected")        
    
    partition = g_ig.community_multilevel()
   
    communities = {node: cluster_id for node, cluster_id in enumerate(partition.membership)}
    
    return communities    
    
    
def get_community_gn(g):     
    g_nx = dgl.to_networkx(g.cpu(), node_attrs=[dgl.NTYPE]).to_undirected()
    partition = nx.community.girvan_newman(g_nx)   
    
    communities = {}
    for comm_id, community in enumerate(next(partition)):
...
def get_neg_path_score_func(g, weight, exclude_node=[]):
    ...
    community=get_community(g)
    neg_path_score_map = {edge: (log_in_degrees[edge[1]] - log_eweights[i]) if community[edge[0]] == community[edge[1]] else (log_in_degrees[edge[1]] - log_eweights[i] +0.1)  for i, edge in enumerate(zip(u.tolist(), v.tolist()))} 
...
def k_shortest_paths_generator(g, 
    ...
    while not prev_path or len(listA) < k:
        if not prev_path:
            ...
            listB.push(length+com_num, path)
            
            
            #print(11,length,com,com_num,length+com_num, path)
            ...
            for i in range(1, len(prev_path)):
                ...
                try:
                    ...
                    com_num = len(set(community[node] for node in path))                    
                    listB.push(root_length + length + com_num, path)
                    
                    #print(12,root_length +length,com,com_num,root_length +length+com_num, path)
...
def bidirectional_dijkstra2(g, src_nid, tgt_nid, weight=None, ignore_nodes=None, ignore_edges=None):
    """Dijkstra's algorithm for shortest paths using bidirectional search.
    ...
    while fringe[0] and fringe[1]:
        ...
        if v in dists[1 - dir]:
            ...
            a_communities = [communities[node] for node in finalpath]
            unique_communities = set(communities[node] for node in finalpath)
            num_unique_communities = len(unique_communities) 
            
            print([a_communities,unique_communities,num_unique_communities,finaldist,finaldist+len(set(communities[node] for node in finalpath)), finalpath])            
            print('m')
            
            
            return (finaldist+len(set(communities[node] for node in homo_path)), finalpath)
            ...
            if w in dists[dir]:
                ...
                if w in seen[0] and w in seen[1]:
                    ...
                    if finalpath == [] or finaldist > totaldist:
                        ...
                        #unique_communities = set(communities[node] for node in finalpath)
                        #num_unique_communities = len(unique_communities)  
                        #finaldist=totaldist+num_unique_communities
                        #print([a_communities,unique_communities,num_unique_communities,totaldist,finaldist, finalpath])            
    ...
    raise ValueError("No paths found")
def k_shortest_paths_generator2(g, 
                               src_nid, 
                               tgt_nid, 
                               weight=None, 
                               k=5, 
    ...
    while not prev_path or len(listA) < k:
        if not prev_path:
            ...
            com_num=get_community_num(g, path)
            listB.push(length+com_num, path)
            print(length, path)
            print(length+com_num, path)
            #print(n) 
            ...
            for i in range(1, len(prev_path)):
                ...
                try:
                    ...
                    com_num=get_community_num(g, path)
                    listB.push(root_length + length+com_num, path)
                    print(root_length + length+com_num, path)
                except ValueError:
                    pass
                ignore_nodes.add(root[-1])
        ...
        if listB:
            ...
            break
def get_community_num2(g, homo_path):
    communities = get_community(g)    
    unique_communities = len(set(communities[node] for node in homo_path))
